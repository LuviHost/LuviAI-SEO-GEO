...
const CONTENT = {
  tr: {
    ...
    s5tiktok: " TikTok Login Kit (user.info.basic) ile yalnızca açık rıza ile alınan kullanıcı kimliği gösterilir; video.publish + video.upload scope'ları yalnızca kullanıcının LuviAI panelinden onayladığı içerikleri kullanıcının kendi TikTok hesabına yayınlamak için kullanılır. LuviAI hiçbir koşulda kullanıcı onayı olmadan otomatik yayın yapmaz. TikTok'tan alınan veri yalnızca kullanıcıya hizmet sunmak için kullanılır, üçüncü taraflara satılmaz, reklam veya profilleme amacıyla işlenmez. Kullanıcı hesabı sildiğinde tüm TikTok token'ları + profil bilgisi 30 gün içinde kalıcı olarak silinir.",
  ...
  en: {
    ...
    s5p1: 'LuviAI connects to social platforms like LinkedIn, X (Twitter), Bluesky, TikTok, YouTube, Facebook, Instagram, Threads, Pinterest via OAuth 2.0. Connection requires explicit user consent and the user can disconnect at any time (Settings → Social Channels → Disconnect).',
    s5label: 'Stored data:',
    s5items: [
