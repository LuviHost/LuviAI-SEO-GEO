...
const SIZES: Record<Size, { px: number; outer: number; inner: number; center: number; tick: number; tickCount: number; strokeOuter: number; strokeArc: number; arcGap: number }> = {
  sm: { px: 44, outer: 20, inner: 14, center: 4, tick: 2, tickCount: 36, strokeOuter: 1, strokeArc: 2, arcGap: 8 },
  md: { px: 140, outer: 64, inner: 48, center: 14, tick: 4, tickCount: 60, strokeOuter: 1.25, strokeArc: 3, arcGap: 14 },
  lg: { px: 220, outer: 100, inner: 76, center: 22, tick: 6, tickCount: 84, strokeOuter: 1.5, strokeArc: 4, arcGap: 22 },
};
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* color arcs (counter-rotate, very slow) */}
        <g
          ref={colorArcsRef}
          style={{ transformOrigin: `${cx}px ${cy}px`, transformBox: 'fill-box' }}
        >
        ...
        {/* fast scan beam (only when scanning) */}
        {state === 'scanning' && (
          <g
            ref={scanBeamRef}
            style={{ transformOrigin: `${cx}px ${cy}px`, transformBox: 'fill-box' }}
      ...
      {/* center content overlay (icon, percent, or children) */}
