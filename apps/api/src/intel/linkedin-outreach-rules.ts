...
export function renderInMailKonu(p: ProspectLike): string {
  const kampanya = normalizeKampanya(p.kampanya);
  const firma = p.firma.trim();
  // NEDEN sade konu: "— ücretsiz karne" gibi basliklar reklam gibi duruyordu (31.08 kullanici geri bildirimi)
  if (kampanya === 'YATIRIMCI') return 'Kısa bir tanışma — RanksUp';
  if (kampanya === 'ISBIRLIGI') return `${firma} için bir fikir`;
  return `${firma} ChatGPT'de nasıl görünüyor?`;
}
...
export function renderInMail(p: ProspectLike): string {
  ...
  if (kampanya === 'YATIRIMCI') {
    return (
      `Merhaba ${ad},\n\n` +
      `RanksUp'ın kurucusuyum. Kısaca ne yaptığımız: insanlar artık ürün ararken Google yerine ChatGPT'ye ` +
      `soruyor ve cevapta bir avuç marka geçiyor. Biz markaların o cevaplarda nerede durduğunu ölçüyor, ` +
  ...
  if (kampanya === 'ISBIRLIGI') {
    return (
      `Merhaba ${ad},\n\n` +
      `Müşterileriniz muhtemelen soruyordur: "ChatGPT bizi öneriyor mu?" Biz RanksUp'ta bunu ölçüyoruz — ` +
  ...
  // pazarlama kalibi ("bagimsiz arastirma yurutuyorum", "karne") yerine somut gozlem + net teklif.
  return (
    `Merhaba ${ad},\n\n` +
